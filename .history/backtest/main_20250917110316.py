...
for i in range(1, len(df)):
    if i >= lookback:
        ...
        median_sell = pd.Series(sell_returns).median()
        sell_threshold = pd.Series(sell_returns[sell_returns >= median_sell]).mean()
        
        # 買い指値
        buy_returns = (df["安値"].iloc[i-lookback+1:i+1].values / df["終値"].iloc[i-lookback:i].values - 1)
        median_buy = pd.Series(buy_returns).median()
        buy_threshold = pd.Series(buy_returns[buy_returns <= median_buy]).mean()
    else:
        sell_threshold = 0.01
        buy_threshold = -0.01
...
plt.title("BTC Backtest with Median-Based Buy/Sell & MA15 Filter")
plt.xlabel("Date")
