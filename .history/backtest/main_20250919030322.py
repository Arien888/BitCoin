...
def backtest_single(args):
    ...
    for i in range(1, len(df)):
        ...
        if i >= lookback:
            ...
            if method.startswith("ma_based"):
                base_method = method.replace("ma_based_", "")
                buy_threshold, sell_threshold = calc_thresholds(pd.Series(buy_returns), pd.Series(sell_returns), base_method)
            elif method != "close" and method != "ma_simple":
        ...
        # 終値優先MAルール
        elif method.startswith("ma_based"):
        ...
        # 単純MAルール
        elif method == "ma_simple":
            if cash > 0 and df["安値"].iloc[i] <= df["MA"].iloc[i]:
                btc = cash / df["MA"].iloc[i]
                cash = 0
            if btc > 0 and df["高値"].iloc[i] >= df["MA"].iloc[i]:
                cash = btc * df["MA"].iloc[i]
                btc = 0

        # その他のルール（mean, median, median_avg）
        else:
...
if __name__ == "__main__":
    ...
    # "ma_simple" を追加して単純MAルールも検証
    methods = ["mean", "median", "median_avg", "close",
               "ma_based_mean", "ma_based_median", "ma_based_median_avg", "ma_simple"]
        ...
        for ma in ma_periods:
            ...
            for method in methods:
                for lookback in lookbacks:
                    tasks.append((to_gpu(df.copy()), method, lookback, ma))

        with ProcessPoolExecutor() as executor:
            futures = {executor.submit(backtest_single, (t[0], t[1], t[2])): t for t in tasks}
            for future in as_completed(futures):
                t = futures[future]
                ma, method, lookback = t[3], t[1], t[2]
    ...
    pd.DataFrame([{"全銘柄平均Lookback日数": avg_lookback}]).to_csv("backtest_best_lookback_avg_parallel_gpu.csv", index=False)
