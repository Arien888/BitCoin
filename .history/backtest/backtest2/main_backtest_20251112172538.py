...
def backtest_full_strategy_repeat(df, ma_period, lookback,
    ...
    for i in range(lookback, len(df)):
        ...
        if buy_trigger:
            ...
            if cur_low <= buy_price:  # 指値買い成立
                if cur_high >= sell_target:  # 同日利確
        ...
        # --- 売りトリガー ---
            ...
            if cur_high >= sell_price:  # 指値売り成立
                if cur_low <= buy_back:  # 同日買い戻し成功
    ...
    if len(total_values) == 0:
        return {"final_value": initial_cash, "profit_percent": 0, "max_drawdown": 0,
                "trade_count": 0, "avg_trade_profit": 0}
