...
def backtest_full_strategy_repeat(df, ma_period, lookback,
                                  ...
                                  max_lot=1.0, min_lot=0.2, range_days=30):
    ...
    for i in range(max(lookback, range_days), len(df)):
        # --- 過去データから閾値計算 ---
        ...
        # --- 30日レンジ位置計算（逆張りスケーリング） ---
        high_n = df["高値"].iloc[i - range_days:i].max()
        low_n = df["安値"].iloc[i - range_days:i].min()
        if high_n == low_n:
            position_scale = 1.0
        else:
            position_pct = (prev_close - low_n) / (high_n - low_n)
            position_scale = max_lot - (max_lot - min_lot) * position_pct  # 安値で強、上値で弱

        trade_amount_dynamic = trade_amount * position_scale
        ...
        if buy_trigger:
            ...
            if cur_low <= buy_price and cur_high >= sell_target:
                ...
                profit = trade_amount_dynamic * (profit_pct / 100)
                total_profit += profit
