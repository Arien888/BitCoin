    ...
    for i in range(1, len(df)):
        ...
        if i >= lookback:
            ...
            if sell_method.startswith("ma_based"):
                ...
                _, sell_threshold = calc_thresholds(
                    pd.Series(buy_returns), pd.Series(sell_returns), base_method
                )
            elif sell_method not in ["close", "ma_simple"]:
                _, sell_threshold = calc_thresholds(
                    pd.Series(buy_returns), pd.Series(sell_returns), sell_method
                )
            else:
                sell_threshold = 0.0
        else:
            ...
            for buy_method in methods:
                for sell_method in methods:
                    for lookback in lookbacks:
                        tasks.append(
                            (to_gpu(df.copy()), buy_method, sell_method, lookback, ma)
                        )
