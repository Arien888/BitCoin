...
# MA期間と指値ルール
ma_periods = list(range(1, 30))  # 15～25
...
for ma in ma_periods:
    for method in methods:
        total_initial = 10000 * len(csv_files)
        total_final = sum(results_all[file][(ma, method)][0] for file in csv_files)
        total_profit = (total_final - total_initial) / total_initial * 100
...
portfolio_df.to_csv("backtest_portfolio_summary.csv", index=False)
...
print("個別結果: 'backtest_summary_all.csv'、")
print("ポートフォリオ合計結果: 'backtest_portfolio_summary.csv'、")
print("最適条件: 'backtest_best_all.csv' に保存しました。")
