...
def backtest_single(args):
    ...
    for i in range(1, len(df)):
        ...
        if i >= lookback:
            buy_returns = (
                df["安値"].iloc[i - lookback + 1 : i + 1].values
                / df["終値"].iloc[i - lookback : i].values
                - 1
            )
            sell_returns = (
                df["高値"].iloc[i - lookback + 1 : i + 1].values
                / df["終値"].iloc[i - lookback : i].values
                - 1
            )
            if method.startswith("ma_based"):
                base_method = method.replace("ma_based_", "")
                buy_threshold, sell_threshold = calc_thresholds(
                    pd.Series(buy_returns), pd.Series(sell_returns), base_method
                )
            elif method != "close" and method != "ma_simple":
                buy_threshold, sell_threshold = calc_thresholds(
                    pd.Series(buy_returns), pd.Series(sell_returns), method
                )
            else:
...
if __name__ == "__main__":
    ...
    for file in csv_files:
        ...
        with ProcessPoolExecutor() as executor:
            futures = {
                executor.submit(backtest_single, (t[0], t[1], t[2])): t for t in tasks
            }
