...
if __name__ == "__main__":
    ...
    # レンジ期間 30～120 日でバックテスト
    results = []
    for range_period in range(30, 121, 10):  # 30,40,...,120
        final, profit = backtest_range_strategy(
            df, MA_Period, Lookback,
            Buy_Method_MA, Buy_Method_Prev, Sell_Method_MA, Sell_Method_Prev,
            initial_cash=initial_cash, range_period=range_period
        )
        results.append((range_period, final, profit))

    # 結果表示
    print("レンジ期間別バックテスト結果")
    print("RangePeriod\tFinalAsset\tProfit%")
    for r, f, p in results:
        print(f"{r}\t{f:.2f}\t{p:.2f}")
