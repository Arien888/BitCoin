...
def place_spot_order(symbol, side, order_type, size, price=None, time_in_force="GTC"):
    ...
    try:
        ...
        print(f"[WARN] {symbol} → price={price}, size={size} → 数値化できずスキップ")
        return

    if price is not None and size * price < 5:
        print(f"スキップ: {symbol} → {size}×{price} = {size * price}USDT（5未満）")
        return

    print(
        f"[DEBUG] symbol: {symbol}, side: {side}, order_type: {order_type}, size: {size}, price: {price}"
    )
    ...
    params = {
        ...
        "quantity": str(size),  # ✅ spot注文は quantity
        "price": str(price),  # LIMIT注文なら price 必須
    }
    ...
    print(f"=== 注文: {symbol} {side} {order_type} {size} @ {price} ===")
...
def read_orders_from_excel(sheet_name):
    ...
    try:
        ...
        for row in range(2, 101):
            ...
            size = sheet.range(f"D{row}").value
