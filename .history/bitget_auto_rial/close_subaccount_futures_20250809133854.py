...
def get_headers(key, secret, passphrase, method, request_path, body=""):
    ...
    headers = {
        ...
        "ACCESS-PASSPHRASE": passphrase_signature,  # 署名済パスフレーズを使う
...
def get_positions(
    ...
    if sub_uid:
        query_params.append(f"subUid={sub_uid}")
    query = "?" + "&".join(query_params)

    request_path_with_query = base_path + query
    url = BASE_URL + request_path_with_query

    print(f"[DEBUG] 使用APIエンドポイントパス: {request_path_with_query}")
    ...
    headers = get_headers(key, secret, passphrase, method, request_path_with_query, body="")
