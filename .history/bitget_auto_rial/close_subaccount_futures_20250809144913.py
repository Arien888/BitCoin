...
def get_positions(
    key, secret, passphrase, sub_uid, base_path="/api/v2/mix/position/all-position"
):
    method = "GET"
...
def cancel_all_orders(key, secret, passphrase, sub_uid, symbol=None):
    method = "POST"
    request_path = "/api/v1/mix/order/cancel-all"
    url = BASE_URL + request_path
...
def main():
    ...
    for pos in positions:
        ...
        position_side = pos.get("holdSide") or pos.get("positionSide")
        qty = float(pos.get("available", 0))

        print(f"[DEBUG] symbol={symbol}, position_side={position_side}, qty={qty}")

        if qty <= 0:
            print(
                f"[INFO] ポジション数量が0または未取得のためスキップ: {symbol} {position_side}"
            )
            continue

        # まず未決済注文をキャンセル
        cancel_all_orders(key, secret, passphrase, sub_uid, symbol)

        # その後ポジションクローズ
        print(
            f"[INFO] ポジションをクローズします: symbol={symbol}, side={position_side}, qty={qty}"
        )
