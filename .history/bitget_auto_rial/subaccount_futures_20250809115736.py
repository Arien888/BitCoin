...
buy_sheet = config["excel"]["sheets"]["bitget_open_long_bigmargin"]
# close_long_sheet = config["excel"]["sheets"].get("close_long")

# BitgetClient インスタンス化（テストネット切り替え対応）
client = BitgetClient(
    key=config["subaccount"]["bitget_Unified"]["api_key"],
    secret=config["subaccount"]["bitget_Unified"]["api_secret"],
    passphrase=config["subaccount"]["bitget_Unified"]["passphrase"],
    is_testnet=config["subaccount"]["bitget_Unified"].get("is_testnet", False),  # ← ここで切り替え
)
