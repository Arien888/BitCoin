...
def close_position(key, secret, passphrase, sub_uid, symbol, position_side):
    ...
    print(f"[DEBUG] リクエストボディ: {body}")

    headers = get_headers(key, secret, passphrase, method, request_path, body)
    print(f"[DEBUG] 署名ヘッダー: {headers}")

    try:
        response = requests.post(url, headers=headers, data=body)
    except Exception as e:
        print(f"[ERROR] リクエスト送信失敗: {e}")
        return None

    print(f"[DEBUG] ステータスコード: {response.status_code}")
    print(f"[DEBUG] レスポンスヘッダー: {response.headers}")

    try:
        data = response.json()
        print(f"[DEBUG] レスポンスJSON: {json.dumps(data, indent=2)}")
    except Exception as e:
