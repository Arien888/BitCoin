...
# --- 設定 ---
CONFIG_FILE = "config.json"  # APIキー等を保存したファイル
...
def main():
    ...
    for pos in positions:
        ...
        if long_size > 0:
            ...
            close_position(api_key, api_secret, api_passphrase, sub_uid, symbol, "sell", long_size)
        if short_size > 0:
            print(f"[INFO] {symbol} ショート {short_size} を買い成行でクローズします...")
            close_position(api_key, api_secret, api_passphrase, sub_uid, symbol, "buy", short_size)
