...
def save_to_csv(products, filename="bitget_futures_products_demo.csv"):
    ...
    with open(filename, "w", newline="", encoding="utf-8") as f:
        ...
        for p in products:
            symbol = p.get("symbol")
            symbol_short = symbol.split("_")[0]
            writer.writerow([symbol_short, p.get("pricePlace"), p.get("volumePlace")])
    ...
    if futures_products.get("code") == "00000":
        ...
        for product in products:
            print(
                product["symbol"],
                "pricePlace:",
                product.get("pricePlace"),
                "volumePlace:",
                product.get("volumePlace"),
            )
