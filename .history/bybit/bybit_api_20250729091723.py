...
def generate_signature_v5(api_secret, payload):
    return hmac.new(
...
def get_bybit_spot_assets(api_key, api_secret, base_url="https://api.bybit.com"):
    ...
    recv_window = "5000"
    query_params = {"accountType": "SPOT"}

    query_string = "&".join(f"{k}={v}" for k, v in sorted(query_params.items()))
    full_payload = f"{timestamp}{api_key}{recv_window}{method}{endpoint}?{query_string}"

    sign = generate_signature_v5(api_secret, full_payload)
