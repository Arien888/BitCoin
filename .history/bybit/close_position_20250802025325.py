...
def generate_signature(
    secret: str, api_key: str, timestamp: str, recv_window: int, data=None
) -> str:
    if data is None:
        params = {}
        ...
        # 文字列の場合は署名対象にできないため空にするか、必要に応じてパースを実装
        params = {}
    ...
    sorted_params_str = urllib.parse.urlencode(sorted(params.items()))
...
def get_open_positions(api_key, api_secret):
    ...
    sign = generate_signature(api_secret, timestamp, method, path, params)
