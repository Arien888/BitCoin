...
def optimize_multi_period(df, months=3):
    ...
    print(f"=== {len(periods)}個の期間で最適化を実施 ===")

    for idx, p in enumerate(periods):
        print(f"\n--- 期間 {idx+1} ---")
        r = optimize_fine(p, top_n=50)   # 期間ごとにTOP50
        all_results.append(r)

    print("\n=== 全期間で強いパラメータ抽出 ===")
    robust = intersect_best(all_results)

    print("\n=== ロバスト（共通強）パラメータ TOP20 ===")
    for r in robust[:20]:
        print(r)
...
def intersect_best(all_results):
    """
    すべての期間で良い評価のパラメータだけを抽出する
    """
    # リストのリスト → 各期間TOPのリスト
    sets = [set(tuple(sorted(r.items())) for r in period) for period in all_results]
