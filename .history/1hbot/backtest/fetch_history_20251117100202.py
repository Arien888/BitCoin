...
def fetch_batch(symbol="BTCUSDT", granularity=3600, limit=200, end_time=None):
    """
    Bitget Futures(MIX) API で過去のローソク足を取得
    end_time: この時刻よりも前のデータを取得（ms）
    """
    params = {
        "symbol": symbol,
...
def fetch_history_months(months=6):
    """
    過去 n か月分の1h足を遡って取得する
    """
    print(f"Fetching ~{months} months of BTCUSDT 1H candles...")
    ...
    while len(all_rows) < target_rows:
        ...
        # 次回は最後の timestamp よりも前へ
        last_ts = batch[-1][0]  # 先頭は timestamp(ms)
