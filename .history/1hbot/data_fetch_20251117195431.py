...
def fetch_1h_candles(cfg, limit=300):
    """
    Bitget UMCBL 1h candles
    正しい endpoint → /api/mix/v1/market/candles
    """
    ...
    params = {
        "symbol": symbol,
        "granularity": 3600,  # 1h
        "size": limit         # ← mix futures は limit ではなく size
    }

    data = bitget_public_get(cfg, path, params)

    # 返ってくる形式は list-of-list
    ...
    # timestamp が ms か ISO の2パターンがあるので両対応
    try:
