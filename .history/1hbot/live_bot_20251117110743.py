...
def bitget_public_request(cfg, path, params=None):
    base_url = cfg["bitget"]["base_url"]
    if params is None:
        params = {}
    query = ""
...
def main():
    ...
    if signal == "BUY" and not has_pos:
        ...
        # ロング全決済（sizeは現在ポジションサイズを使う方が良いが、簡略化して calc_order_size でも可）
        size = calc_order_size(cfg, last["close"])
        ...
        if cfg["mode"]["dry_run"]:
            ...
            print("ORDER RESPONSE:", res)
            append_trade_log("ORDER", "close_long", size, last["close"], extra=json.dumps(res))

    else:
        print("No action. HOLD.")
        append_trade_log("HOLD", "-", 0, last["close"])
