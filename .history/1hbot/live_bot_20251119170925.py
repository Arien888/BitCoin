...
def main():
    try:
        ...
        if signal == "BUY" and not has_pos:
            ...
            size = calc_order_size(cfg, last["close"])     # ← 先に size

            write_runtime_log(f"SELL signal | size={size} | price={last['close']}")
            write_trade_log("close_long", size, last["close"], "DRY_RUN" if cfg["mode"]["dry_run"] else "ORDER")
