...
# --- Excel ファイルパス ---
excel_path = os.path.abspath(os.path.join(BASE_DIR, "..", config["order_export"]["source_file"]))
...
try:
    ...
    # 文字列化して送信
    str_df = df.fillna("").astype(str)  # NaN は空文字に置換
    worksheet.update(
    ...
    # シートがなければ新規作成
    worksheet = spreadsheet.add_worksheet(title=safe_sheet_name, rows=df.shape[0]+10, cols=df.shape[1]+10)
