...
def main():
    ...
    api_key = config["bitget"]["bitget_Unified"]["api_key"]
    api_secret = config["bitget"]["bitget_Unified"]["api_secret"]
    api_passphrase = config["bitget"]["bitget_Unified"]["passphrase"]
    ...
    for asset_type, path in urls.items():
        ...
        try:
            ...
            write_to_existing_excel(
                excel_path, result, keys, sheet_name=f"{asset_type}_sub"
            )
