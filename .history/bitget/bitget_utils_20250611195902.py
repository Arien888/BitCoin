...
def save_assets_to_csv_jp(filename, data):
    ...
    with open(filename, mode="w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        # 例: スポット資産の場合
        save_assets_to_csv_jp("spot_assets.csv", spot_data, spot_keys)

        # マージン資産の場合
        save_assets_to_csv_jp("margin_assets.csv", margin_data, margin_keys)

        # イールド資産の場合
        save_assets_to_csv_jp("earn_assets.csv", earn_data, earn_keys)
