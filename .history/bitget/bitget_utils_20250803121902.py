...
def get_futures_positions(
    ...
    # product_typeが有効値の場合のみクエリに追加
    valid_product_types = {"UMCBL", "CMCBL", "UMCSP", "CMCSL"}  # 例、必要に応じて更新
    query_string = ""
    if product_type and product_type.upper() in valid_product_types:
        query_string = f"?productType={product_type.upper()}"
    ...
    if response.status_code != 200:
        try:
            ...
            print(f"HTTP Error: {response.status_code} - {err_json}")
        except json.JSONDecodeError:
            print(f"HTTP Error: {response.status_code} - Response not JSON")
        return None
